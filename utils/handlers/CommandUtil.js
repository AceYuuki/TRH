...
module.exports = async client => {
    (await pGlob(`${process.cwd()}/commands/*/*.js`)).map(async cmdFile => {
        ...
        if (!cmd.usage) return Logger.warn(`Commande non-chargé : pas d'utilisation (usage)' ⇩\nFichier -> ${cmdFile}`);

        if (!cmd.exemples) return Logger.warn(`Commande non-chargé : pas d'exemples ⇩\nFichier -> ${cmdFile}`);

        cmd.permissions.forEach(permission => {
            if (!permissionList.includes(permission)) {
                return Logger.typo(`Commande non-déclanché : erreur de typo sur la description ${permission} ⇩\nFichier -> ${cmdFile}`);
            }
        });
